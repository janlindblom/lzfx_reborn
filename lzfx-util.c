...
static void fx_init(FX_STATE* state, int ifd, int ofd, fx_mode_t mode) {
    state->ifd  = ifd;
    state->ofd  = ofd;
    state->mode = mode;
}
...
/*  Read len bytes from the input file.
    ...
    >=0: bytes read, either 0 (EOF) or len
     <0: Read error
*/
static inline int fx_read_bytes(const FX_STATE state, void* buf, const size_t len) {
    ssize_t rc    = 0;
    size_t  count = 0;
...
/*  Write len bytes from the buffer to the output file.
    ...
    >=0:    Bytes written
     <0:    Write error
*/
static inline int fx_write_bytes(const FX_STATE state, const void* buf, const size_t len) {
    ssize_t rc    = 0;
    size_t  count = 0;
...
/* Skip len bytes in the input file.
    ...
    len:    # of bytes to skip
     ...
     <0:    Read error
*/
static int fx_skip_bytes(const FX_STATE state, const size_t len) {
    off_t rc;
...
*/
static inline int fx_read_header(const FX_STATE state, fx_kind_t* kind_in, uint32_t* len_in) {
    int      rc;
    uint16_t kind;
    uint32_t len;
    uint8_t  head[10];
